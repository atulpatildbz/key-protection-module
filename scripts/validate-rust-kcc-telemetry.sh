...
scan_files() {
  ...
  while IFS= read -r file; do
    ...
    violations="$(grep -En -- "$FORBIDDEN_PATTERN" "$file" || true)"
...
scan_path() {
  ...
  if [[ -f "$path" ]]; then
    ...
    find "$path" -type f -name '*.rs' -print | sort | scan_files
  fi
}
...
scan_telemetry_module() {
  ...
  logging_count="$(grep -Eo -- "$LOGGING_PATTERN" "$path" | wc -l | tr -d ' ')"
  approved_logging_count="$(grep -Eo -- 'tracing::error[[:space:]]*!' "$path" | wc -l | tr -d ' ')"
  hook_count="$(grep -Eo -- 'std::panic::set_hook[[:space:]]*\(' "$path" | wc -l | tr -d ' ')"
  direct_formatting="$(grep -En -- "$DIRECT_FORMAT_PATTERN" "$path" || true)"
